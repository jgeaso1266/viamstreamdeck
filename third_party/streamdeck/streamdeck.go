...
// Touchscreen LCD strip geometry and HID protocol constants for the Stream Deck
// Plus. The strip is a single 800x100 JPEG surface written with output report
// 0x02 command 0x0c, in 1024-byte packets carrying a 16-byte header and up to
// 1008 bytes of JPEG payload each. This is a different report than the button
// image writer (0x02 0x07), which is why FillImage/FillPanel cannot drive it.
const (
	TouchscreenWidth      = 800
	TouchscreenHeight     = 100
	touchscreenPacketLen  = 1024
	touchscreenHeaderLen  = 16
	touchscreenPayloadLen = touchscreenPacketLen - touchscreenHeaderLen // 1008
)

// SetTouchscreenImage writes img to the rectangle (x,y,w,h) of the Stream Deck
// Plus touchscreen LCD strip. img is JPEG-encoded and sent as chunked 0x02 0x0c
// reports over the same HID handle used for buttons; the write is serialized
// with sd.lock so a multi-packet strip frame can't interleave with a button
// update mid-transfer. This is an addition over upstream dh1tw/streamdeck, which
// has no touchscreen support.
func (sd *StreamDeck) SetTouchscreenImage(x, y, w, h int, img image.Image) error {
	var jpegBuf bytes.Buffer
	if err := jpeg.Encode(&jpegBuf, img, &jpeg.Options{Quality: 90}); err != nil {
		return fmt.Errorf("failed to encode touchscreen jpeg: %w", err)
	}
	...
	for _, buf := range touchscreenPackets(x, y, w, h, payload) {
		n, err := sd.device.Write(buf)
		if err != nil {
			return err
		}
		if n != len(buf) {
			return fmt.Errorf("touchscreen only wrote %d of %d", n, len(buf))
		}
	}
...
// touchscreenPackets builds the 0x02 0x0c output reports that write payload to
// rectangle (x,y,w,h) of the strip. Each packet is a full touchscreenPacketLen
// buffer with a 16-byte header and up to touchscreenPayloadLen JPEG bytes; the
// page number increments and the final packet sets the is-last flag.
func touchscreenPackets(x, y, w, h int, payload []byte) [][]byte {
	packets := [][]byte{}
	...
	for bytesLeft > 0 {
		chunk := bytesLeft
		if chunk > touchscreenPayloadLen {
			chunk = touchscreenPayloadLen
		}
		bytesLeft -= chunk

		buf := make([]byte, touchscreenPacketLen)
		buf[0] = 0x02
		buf[1] = 0x0c
		binary.LittleEndian.PutUint16(buf[2:], uint16(x))
		binary.LittleEndian.PutUint16(buf[4:], uint16(y))
		binary.LittleEndian.PutUint16(buf[6:], uint16(w))
		binary.LittleEndian.PutUint16(buf[8:], uint16(h))
		if bytesLeft == 0 {
			buf[10] = 1
		}
		binary.LittleEndian.PutUint16(buf[11:], page)
		binary.LittleEndian.PutUint16(buf[13:], uint16(chunk))
		// buf[15] stays 0 (padding)

		copy(buf[touchscreenHeaderLen:], payload[pos:pos+chunk])
...
// FillTouchscreen draws img across the entire 800x100 strip.
func (sd *StreamDeck) FillTouchscreen(img image.Image) error {
	return sd.SetTouchscreenImage(0, 0, TouchscreenWidth, TouchscreenHeight, img)
}
